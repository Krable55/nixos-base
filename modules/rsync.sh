...
# my rsync-based system snapshot backup procedure
# (cc) (Nice Micro)
# based on the script seen at wiki.archlinux.org/index.php/Rsync
...
# we define the source of the backup (/ for full system backup, trailing / for
# backup of the contents of the dir but not making the dir itself)
SRC="/"

# we define the target backup on a different drive, mounted at media (no trailing /)
BCKP="/media/data/backup"

# We exclude the irrelevant system directories, mountpoints and the home folder
# expands into the array of folder names.
...
MANUALP="manual"	# relative path to the manual backups
DAILYP="daily"		# relative path to the daily backups
WEEKLYP="weekly"	# relative path to the weekly backups
MONTHLYP="monthly"	# relative path to the monthly backups
LOGSP="logs"		# relative path to the logs
...
# run rsync
# Using the expansion of the EXCLUDE array here, we get all elements prepended by --exclude and
# also each element in the array will be its own command line parameter to rsync.
echo Running: rsync $OPTS "${EXCLUDE[@]/#/--exclude=}" "$LINK" "$SRC" "$BCKP/$FOLDER/$NAME/" >> "$LOG"
echo rsync $OPTS "${EXCLUDE[@]/#/--exclude=}" "$LINK" "$SRC" "$BCKP/$FOLDER/$NAME/"
rsync $OPTS "${EXCLUDE[@]/#/--exclude=}" "$LINK" "$SRC" "$BCKP/$FOLDER/$NAME/" >> "$LOG" || echo "rsync failed." >> "$LOG"
...
# let's move around the backups if needed
...
# we check if there are too many daily backups now, and remove the last if it is so.
