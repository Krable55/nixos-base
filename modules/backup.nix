...
let
  ...
  backupScript = ''
    ...
    for d in "$DAILYP" "$WEEKLYP" "$MONTHLYP" "$LOGSP"; do
      mkdir -p "$BCKP/$d"
    done

    TODAY=$(find "$BCKP/$DAILYP" -maxdepth 1 -mindepth 1 -type d -ctime -1 -printf "a" | wc -c)
    if [ "$TODAY" -gt 0 ]; then
    ...
    rsync -aAXiH ${includes} --include='*/' --exclude='*' $LINK "$SRC/" "$BCKP/$DAILYP/$NAME/" >> "$LOG" 2>&1 || echo "rsync failed" >> "$LOG"
    ...
    ln -s "$BCKP/$DAILYP/$NAME" "$BCKP/$LAST" 2>>"$LOG"
